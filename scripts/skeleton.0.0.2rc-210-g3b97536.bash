...
#
# Interact with skeleton-ci/skeleton-python (current version: https://github.com/skeleton-ci/skeleton-python/tree/0.0.2rc-210-g3b97536).
#
# This file was generated from skeleton-ci/skeleton-python@0.0.2rc-210-g3b97536.
# Instead of changing this particular file, you might want to alter the template:
# https://github.com/skeleton-ci/skeleton-python/tree/0.0.2rc-210-g3b97536/project/scripts/skeleton.%7B%7Bsref%7D%7D.bash.jinja
#
# Usage:
#
...
make_context() {
	...
	export REPO_URL
	REPO_URL=$(cat <<- 'EOF'
		https://github.com/bswck/pj
	EOF
	)
	export COVERAGE_URL
	COVERAGE_URL=$(cat <<- 'EOF'
		https://coverage-badge.samuelcolvin.workers.dev/redirect/bswck/pj
	EOF
	)
	export DOCS_URL
	DOCS_URL=$(cat <<- 'EOF'
		https://pj.readthedocs.io/en/latest/
	EOF
	)
	export PYPI_URL
	PYPI_URL=$(cat <<- 'EOF'
		https://pypi.org/project/pj/
	EOF
	)
	export TIDELIFT_URL
	TIDELIFT_URL=$(cat <<- 'EOF'
		https://tidelift.com/subscription/pkg/pypi-pj?utm_source=pypi-pj
	EOF
	)
	export SKELETON
	SKELETON=$(cat <<- 'EOF'
	...
	SKELETON_REV=$(cat <<- 'EOF'
		https://github.com/skeleton-ci/skeleton-python/tree/0.0.2rc-210-g3b97536
	EOF
	)
	export SREV
	SREV=$(cat <<- 'EOF'
	...
	SKELETON_AND_REF=$(cat <<- 'EOF'
		skeleton-ci/skeleton-python@0.0.2rc-210-g3b97536
	EOF
	)
	export SNREF
	SNREF=$(cat <<- 'EOF'
	...
	GH_REPO_ARGS=$(cat <<- 'EOF'
		"bswck/pj" --public --source=./ --remote=upstream --description="Set up projects for development quickly."
	EOF
	)
	export GH_ENSURE_ENV
	GH_ENSURE_ENV=$(cat <<- 'EOF'
		jq -n '{"deployment_branch_policy": {"protected_branches": false,"custom_branch_policies": true}}' | gh api -H "Accept: application/vnd.github+json" -X PUT "/repos/bswck/pj/environments/$1" --silent --input -
	EOF
	)
    export LAST_REF="0.0.2rc-210-g3b97536"
